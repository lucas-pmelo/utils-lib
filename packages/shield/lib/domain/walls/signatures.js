...
export const signatures = [
    ...
    { wallId: "xss", pattern: /\bdocument\s*\.\s*cookie\b/i },
    {
        ...
        pattern: /[;&|]{1,2}\s*(rm|cat|ls|curl|wget|nc|ncat|bash|sh|zsh|python|perl|ruby|chmod|chown|kill|shutdown|reboot|mkfs|dd|whoami|id)\b(?=\s*$|\s*[;&|]|\s+[-\/~.$'"]|\s+\w{1,16}:\/\/)/i,
    },
    { wallId: "command-injection", pattern: /\$\(\s*[a-z_.\/]/i },
    {
        wallId: "command-injection",
        pattern: /`\s*(\/(bin|usr|sbin|etc)\/|rm|cat|curl|wget|nc|ncat|bash|sh|zsh|python|perl|ruby|chmod|chown|kill|whoami|uname|eval|export)\b[^`\n]{0,64}`/i,
    },
    ...
    {
        wallId: "prompt-injection",
        pattern: /\byou\s+are\s+now\s+(in\s+)?(dan\b|developer\s+mode|jailbroken|unrestricted|unfiltered|uncensored|free\s+from|no\s+longer\s+bound|an?\s+(unrestricted|unfiltered|uncensored|evil|amoral)\b)/i,
    },
    ...
    {
        wallId: "jailbreak",
        pattern: /\bwithout\s+(any\s+)?(censorship|filters?|ethical\s+guidelines|moral\s+guidelines)\b/i,
    },
    {
        wallId: "jailbreak",
...
const NO_PATTERNS = [];
export function signaturesFor(wallId) {
