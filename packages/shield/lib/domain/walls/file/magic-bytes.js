...
export function detectMimeFromMagic(head) {
    return SIGNATURES.find((signature) => head.length >= signature.bytes.length &&
        signature.bytes.every((byte, index) => head[index] === byte))?.mimeType;
}
